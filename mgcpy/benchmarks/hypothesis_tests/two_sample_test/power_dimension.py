...
def power_given_data(independence_test, simulation_type, num_samples=100, num_dimensions=1, translate=0, theta=0, repeats=1000, alpha=.05, additional_params={}):
    ...
    for rep in range(repeats):
        ...
        matrix_Y = np.zeros((num_samples,num_dimensions))
        ...
        sim_list = [10, 12, 13, 14, 18]
        if simulation_type in sim_list:
            q = make_rot(num_dimensions*2)
        else: 
            rot = np.zeros((num_dimensions*2,1))
            for i in range(num_dimensions*2):
                add=0
                mat = np.zeros((num_dimensions*2,1))
        ...
        trans_Y = np.zeros((num_samples,num_dimensions))
        for dot in range(num_samples):
            ...
            for dime in range(1,num_dimensions):
                ...
                trans_Y[dot][dime] = r_trans[dot][num_dimensions+dime]
